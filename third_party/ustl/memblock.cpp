...
extern "C" unsigned int __clzdi2(unsigned long long x)
{
    if (x == 0)
        return 64;
...
void memblock::reserve (size_type newSize, bool bExact)
{
  ...
  return;
    ...
    if (!bExact)
	newSize = alignedSize;
    pointer newBlock = static_cast<pointer> (mem.realloc(oldBlock, newSize));
    if (!newBlock){
      kprintf("bad_alloc");
      while(1){}
    }
...
void memblock::shrink_to_fit (void)
{
    if (is_linked())
	return;
    pointer newBlock = static_cast<pointer> (mem.realloc(begin(), size()));
    if (!newBlock && size()){
      kprintf("bad_alloc");
      while(1){}
    }
